...
- **Images inside list items and table cells no longer vanish from the PDF.** A picture in a bullet or numbered item is now printed as its own image block behind the item's text; in a table cell, which has no room for a picture, it shows as a visible `[Image: name]` text instead of disappearing (the export notice used to count them as "simplified" while the PDF silently lacked them). **Visible change:** notes with images in lists now get larger PDFs, and tables with images show `[Image: …]` where the cell used to be empty. From `obsidian-kit` 0.42.0 (`pdf/dom-to-ir`); the word comes from the plugin's own catalogue (`pdf.placeholder.image`), so it follows the interface language.
